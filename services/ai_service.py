...
class AIService:
    ...
    def _get_extraction_prompt(self) -> str:
        ...
        # البروميت المحدث حسب الدليل الجديد
        prompt = f"""
دليل تحليل بيانات العقارات (تنسيق البيان الكامل كحقل واحد)

يهدف هذا الدليل إلى توضيح كيفية تحليل بيانات العقارات وتنسيقها لتتوافق مع المعايير المطلوبة. يجب أن يكون الناتج في شكل كود جيسون مع البيان العقاري الكامل، مع كل حقل وقيمته داخل قوسين مربعين، وكل حقل وقيمته في سطر جديد داخل نفس الحقل الكبير.
...
يجب أن يكون الخرج في شكل JSON يحتوي على الحقول التالية مع حقل البيان المدمج:
"""
        return prompt
